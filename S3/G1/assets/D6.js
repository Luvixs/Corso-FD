...
function unisci(str1, str2) {
  let nuova1 = str1.slice(0, 2);
  let nuova2 = str2.slice(-3);
  console.log(nuova1.concat(nuova2).toUpperCase());
}
unisci("hello", "world");
// slice()
...
//console.log(getNumeriRandom());

/* ESERCIZIO 3 (filter)
  Scrivi una funzione per ricavare solamente i valori PARI da un array composto da soli valori numerici
*/
let numeriRandom = getNumeriRandom();
let numeriPari = getNumeriPari(numeriRandom);

function getNumeriPari(Numeri) {
  return Numeri.filter((x) => x % 2 == 0);
}
...
*/
const numeriSomma = [5, 6, 9, 10, 20, 70, 45];

function nuovaSomma(arr) {
  let risultato = 0;
  numeriSomma.forEach((element) => {
    risultato += element;
  });
  return risultato;
}
console.log(nuovaSomma(numeriSomma));
/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
